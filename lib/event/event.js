...
function processEvent(logPrefix, req, res, source, serviceInstanceId, toolchainId, payload) {
	...
	db.get(serviceInstanceId, null, function(err, body) {
		if(err && err.statusCode !== 404) {
			...
			if (!translator) {
				logger.debug(logPrefix + "No event to pagerduty message translator found for source: \"" + source + "\"");
				res.status(204).json({});
				return;
			} else {
				logger.debug(logPrefix + "Processing event: " + JSON.stringify(payload));
				message = translator(payload);
			}
			
			if (!message) {
				// event not critical, do not trigger an alert
				logger.debug(logPrefix + "Event NOT sent to PagerDuty since not critical");
				res.status(204).json({});
				return;
			}
