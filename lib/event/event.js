	...
	db.get(serviceInstanceId, null, function(err, body) {
		if(err && err.statusCode !== 404) {
			...
			if (!translator) {
				...
				var toolchain_id = _.findWhere(body.toolchain_ids, {id: toolchainId});
				var toolchainCredentials = null;
				if (toolchain_id) {
					toolchainCredentials = toolchain_id.credentials;
				}
					
				message = translator(toolchainId, payload, toolchainCredentials, function(err, message) {
					if (err) {
						logger.debug(logPrefix + "Event: " + JSON.stringify(payload));
