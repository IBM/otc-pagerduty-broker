...
.post("/:source/service_instances/:sid", incomingPipelineEvent, getServiceInstance, checkCredentials, processEvent)
...
/*
 * Process an event coming from toolchain.
 * 
 * Note: We are currently not doing anything when receiving notification from otc-api
 * because this is coming from LMS to post to PagerDuty.
 * However we should use this endpoint for the functional part later on
 * and also local dev environment because LMS is not in place in this kind of test.
 */
...
/*
 * Process an event coming from pipeline.
 * 
 * Note: We are currently not doing anything when receiving notification from pipeline.
 * because this is coming from LMS to post to PagerDuty.
 * However we should use this endpoint for the functional part later on
 * and also local dev environment because LMS is not in place in this kind of test.
 */
function incomingPipelineEvent(req, res, next) {
	var logPrefix = "[" + logBasePath + ".incomingPipelineEvent] ";
	...
	if (process.env.NODE_ENV == "local-dev") {
		...
		incomingEvent.serviceInstanceId = serviceInstanceId;
		// incomingEvent.toolchainId = toolchainId; //TODO: Retrieve the toolchain id and name here - a service may be bound to multiple toolchains
		incomingEvent.payload = req.body;
		req.incomingEvent = incomingEvent;
...
/*
 * Process an event coming from LMS.
 */
...
function processEvent(req, res, next) {
	...
	if (!translator) {
		...
		message = translator(toolchainId, payload, toolchainCredentials, function(err, message, notSentReason) {
			...
			pagerdutyUtil.postAlert(service_key, message, function(err, response) {
				if (err) {
					logger.debug(logPrefix + "Event: " + JSON.stringify(payload));
