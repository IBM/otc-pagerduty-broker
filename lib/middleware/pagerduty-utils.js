...
function getOrCreateEscalationPolicy(res, apiUrl, api_key, user_email, user_phone, callback) {
...
function getOrCreatePagerDutyService(res, apiUrl, api_key, service_name, user_email, user_phone, callback) {
	...
	request.get({
		...
		if(service) {
			...
			getOrCreateEscalationPolicy(res, apiUrl, api_key, user_email, user_phone, function(escalation_policy) {
				if (!escalation_policy)
					return callback(null);
