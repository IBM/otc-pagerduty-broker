...
function addPhoneContact(apiUrl, headers, user, phone, type, callback) {
	...
	request.post({
		...
		if (err) {
			...
			logger.error(logPrefix + "Creating the contact method '" + type + "' for"
					+ " user_email: " + user.email + ", country_code:" + phone.country + ", number:" + phone.number
...
function addNotificationRule(apiUrl, headers, user, contact_method, callback) {
	...
	request.post({
		...
		if (err) {
			...
			var message = getErrorMessageFromResult(reqRes);
			logger.error(logPrefix + "Creating the " +
					"notification rule " + type + " for"
					+ " user_email: " + user.email 
					+ " failed with the following error: " + message);
...
function getUser(res, apiUrl, api_key, user_id, callback) {
	...
	request.get({
		...
		if (err) {
			logger.error(logPrefix + "Unable to get user"
					+ " user_id: " + user_id 
					+ " failed with the following error: " + JSON.stringify(err));
			res.status(500).json({ "description": JSON.stringify(err) });
			return callback(null, null);
			...
			// Unexpected error that is not 404 nor 200
			var message = getErrorMessageFromResult(reqRes);
			logger.error(logPrefix + "Unexpected error while getting the user"
					+ " user_id: " + user_id 
					+ " failed with the following error: " + message);
			res.status(500).json({ "description": JSON.stringify(reqRes) });
			return callback(null, null);
...
function getUserInfo(res, apiUrl, api_key, user_id, callback) {
	...
	request.get({
		...
		if (err) {
			...
			// Unexpected error that is not 404 nor 200
			var message = getErrorMessageFromResult(reqRes);
			logger.error(logPrefix + "Unexpected error while getting the userinfo"
					+ " user_id: " + user_id 
					+ " failed with the following error: " + message);
			res.status(500).json({ "description": JSON.stringify(reqRes) });
			return callback(null, null);
		...
		if(user) {
			...
			request.post({
				...
				if (err) {
					...
					res.status(500).json({ "description": JSON.stringify(err) });
					return callback(null);
				} else if(reqRes && reqRes.statusCode >= 400 && reqRes.statusCode < 500) {
					// Invalid request
					var message = getErrorMessageFromResult(reqRes);
					logger.error(logPrefix + "Creating the PagerDuty user for"
							+ " user_email: " + user_email 
							+ " failed [" + reqRes.statusCode + "] with the following error: " + message);
				...
				return getEscalationPolicy(res, apiUrl, api_key, service.escalation_policy.id, function(escalationPolicy) {
					...
					getUserInfo(res, apiUrl, api_key, userId, function(email, phone) {
						if (user_email != email || user_phone != phone) {
							return updateEscalationPolicy(res, apiUrl, api_key, service.escalation_policy.id, escalationPolicy.escalation_rules, user_email, user_phone, patching, function(escalation_policy) {
								if (!escalation_policy) 
									return callback(null);
			...
			getOrCreateEscalationPolicy(res, apiUrl, api_key, service_name, user_email, user_phone, function(escalation_policy) {
				...
				request.post({
					...
					if (err) {
						...
						var message = getErrorMessageFromResult(reqRes);
						logger.error(logPrefix + "Creating the PagerDuty service for"
								+ " service_name: " + service_name + ", escalation_policy_id: " + escalation_policy.id + " and type:" + "generic_events_api"
								+ " failed [" + reqRes.statusCode + "] with the following error: "  + message + ". " + JSON.stringify(reqRes.body));
						res.status(400).json({ "description": message});
						return callback(null);						
					} else if(reqRes && reqRes.statusCode != 201) {
						var message = getErrorMessageFromResult(reqRes);
			...
			request.put({
				...
				if (isOwnContact && !updateUser) {
					...
					request.put({
						...
						if (err) {
							logger.error(logPrefix + "Updating the user name for user with"
									+ " user_email: " + user.email 
									+ " failed with the following error: " + JSON.stringify(err));
							res.status(500).json({ "description": JSON.stringify(err) });
							return callback(null)
