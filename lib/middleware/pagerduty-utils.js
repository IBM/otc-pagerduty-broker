	...
	request.get({
		...
		if (err) {
			...
			for (var i=0; i < body.escalation_policies.length; i++) {
				var escalation_policy = body.escalation_policies[i];
				if (escalation_policy.name == escalation_name) {
					policyExists = true;
				}
			}
		} else if(reqRes.statusCode != 404) {
...
function postAlert(logPrefix, service_key, description, callback) {
	var url = "disabled"; //"https://events.pagerduty.com/generic/2010-04-15/create_event.json";
	request.post({
		uri: url,
		json: true,
		headers: {},
