	...
	db.get(serviceInstanceId, null, function(err, body) {
		if(err && err.statusCode !== 404) {
			...
			**/
			...
			return createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, parametersData, false/*not patching*/)
		} else {
			/**
...
function createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, parametersData, patching) {
	...
	if (account_id && api_key) {
		...
		pagerDutyUtils.getOrCreatePagerDutyService(res, apiUrl, api_key, account_id, service_name, user_email, user_phone, patching, function(service) {
			if (!service)
				return;
...
function patchServiceInstance(req, res /*, next*/) {
	...
	db.get(sid, null, function(err, body) {
		...
		return createOrUpdateServiceInstance_(res, req, db, sid, organizationId, params, true/*patching*/);
	});
}
