...
function createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, parametersData) {
	...
	if (account_id && api_key) {
		...
		pagerDutyUtils.getOrCreatePagerDutyService(res, apiUrl, api_key, account_id, service_name, user_email, user_phone, function(service) {
			if (!service)
				return;
...
function patchServiceInstance(req, res /*, next*/) {
	...
	logger.info(logPrefix + "Patching the service instance with ID: " + sid);
