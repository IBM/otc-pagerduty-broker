...
function createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, serviceCredentials, parametersData, patching) {
	...
	if (account_id && api_key) {
		...
		pagerDutyUtils.getOrCreatePagerDutyService(res, apiUrl, api_key, account_id, service_name, user_email, user_phone, patching, function(err, service) {
			...
			tiamUtil.setServiceData(serviceCredentials, {api_key: api_key, service_key: service.service_key}, function(err, result) {
				if (err) {
					logger.error(logPrefix + "Error while storing API and service keys in TIAM Service:" + err);
					return res.status(500).json({ "description": "Error while storing API and service key." });
				}
				...
				if (!user_email && !user_phone) {
					...
					return pagerDutyUtils.getEscalationPolicy(res, apiUrl, api_key, service.escalation_policy.id, function(err, escalationPolicy) {
						...
						pagerDutyUtils.getUserInfo(res, apiUrl, api_key, userId, function(err, email, phone) {
							...
							return doServiceUpdate(res, req, db, serviceInstanceId, serviceCredentials, parameters, api_key, service.id, organizationId, dashboardUrl);
						});
					});
				}
		...
		return doServiceUpdate(res, req, db, serviceInstanceId, serviceCredentials, parameters, api_key, "n/a", organizationId, "https://www.pagerduty.com");
	}	
}
...
**/
function doServiceUpdate (res, req, db, serviceInstanceId, serviceCredentials, parametersData, api_key, instanceId, organizationId, dashboardUrl) {
	...
	return nanoDocUpdater()
		...
		.update(function (err) {
				...
				parametersData.api_key = api_key;
				return res.json({
					instance_id: instanceId,
					dashboard_url: dashboardUrl,
					parameters: parametersData,
	...
	db.get(sid, null, function(err, body) {
    	...
    	async.waterfall([
			function findData(callback) {
			    if (params.api_key) {
			    	callback(null, {api_key: params.api_key});
			    } else {
					tiamUtil.getServiceData(serviceCredentials, callback);
			    }
			},
			function (data, callback) {
				tiamUtil.setServiceData(body.service_credentials, {api_key: data.api_key}, function(err, result) {
					if (err) {
						logger.error(logPrefix + "Error while storing API Key in TIAM Service:" + err);
						return res.status(500).json({ "description": "Error while storing API Key." });
					}
