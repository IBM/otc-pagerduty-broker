...
function createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, parametersData) {
	...
	if (account_id && api_token) {
		...
		if (index == -1) {
			return res.status(400).json({ "description": "Invalid pagerduty service: " + baseUrl});
		}
		var url = httpsPrefix + account_id + '.' + baseUrl.substring(httpsPrefix.length);
		var apiUrl = url + "/api/v1";
		pagerDutyUtils.getOrCreatePagerDutyUser(apiUrl, api_token, user_email, user_phone, function (err, user) {
			if (err) {
				return res.status(400).json({ "description": "Error: Unable to find or create user (" + err.toString() + ")" });
			}
			pagerDutyUtils.getOrCreateEscalationPolicy(apiUrl, api_token, user, function (err, escalation_policy) {
				...
				pagerDutyUtils.getOrCreatePagerDutyService(apiUrl, api_token, service_name, escalation_policy, function (err, service) {
					if (err) {
						return res.status(400).json({ "description": "Error: Unable to find or create PagerDuty service (" + err.toString() + ")" });							
					} 
					var dashboardUrl = url + service.service_url;
					parameters.label = account_id;
