...
function createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, serviceCredentials, parametersData, patching) {
	...
	if (site_name && api_key) {
		...
		pagerDutyUtils.getOrCreatePagerDutyService(res, apiUrl, api_key, site_name, service_name, user_email, user_phone, patching, function(err, service) {
			if (err)
				return;
...
function patchServiceInstance(req, res /*, next*/) {
	...
	var allowedParams = ["site_name", /*temporary while catalog is not updated*/"account_id", "api_key", "service_name", "user_email", "user_phone"];
	...
	params = _.pick(params, "site_name", /*temporary while catalog is not updated*/"account_id", "api_key", "service_name", "user_email", "user_phone");
