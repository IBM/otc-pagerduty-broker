...
function createOrUpdateServiceInstance_(res, req, db, serviceInstanceId, organizationId, serviceCredentials, parametersData, patching) {
	...
	if (site_name && api_key) {
		...
		var url = httpsPrefix + site_name + '.' + baseUrl.substring(httpsPrefix.length);
		var apiUrl = url + "/api/v1";
		pagerDutyUtils.getOrCreatePagerDutyService(res, apiUrl, api_key, site_name, service_name, user_email, user_phone, patching, function(err, service, existing) {
			if (err)
				return;
			...
			if (existing) {
				...
				return pagerDutyUtils.getEscalationPolicy(res, apiUrl, api_key, service.escalation_policy.id, function(err, escalationPolicy) {
					...
					pagerDutyUtils.getUserInfo(res, apiUrl, api_key, userId, function(err, email, phone) {
						...
						return doServiceUpdate(res, req, db, serviceInstanceId, serviceCredentials, parameters, service.id, organizationId, dashboardUrl, patching);
					});
				});
			}
		...
		return doServiceUpdate(res, req, db, serviceInstanceId, serviceCredentials, parameters, "n/a", organizationId, "https://www.pagerduty.com", patching);
	}	
}
...
**/
function doServiceUpdate (res, req, db, serviceInstanceId, serviceCredentials, parametersData, instanceId, organizationId, dashboardUrl, patching) {
	...
	if (patching) {
		toolchain_ids = null; // do not override existing toolchain_ids
	}
