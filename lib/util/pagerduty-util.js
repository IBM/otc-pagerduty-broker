...
var logger = log4js.getLogger("otc-pagerduty-broker"),
 	logBasePath = "lib.util.pagerduty-util";
...
function addPhoneContact(apiUrl, headers, user, phone, type, callback) {
	...
	logger.debug(logPrefix + "phone:" + JSON.stringify(phone) + " user: " + JSON.stringify(user) + " type: " + type);
...
function getEscalationPolicy(res, apiUrl, api_key, escalation_id, callback) {
	...
	logger.debug(logPrefix + " escalation_id: " + escalation_id);
	var headers = {
...
function getUser(res, apiUrl, api_key, user_id, callback) {
	...
	logger.debug(logPrefix + " user_id: " + user_id);
	var headers = {
		'Authorization': 'Token token=' + api_key,
		'user-agent': 'node.js'
	};
...
function getOrCreatePagerDutyUser(res, apiUrl, api_key, userName, user_email, user_phone, callback) {
	...
	logger.debug(logPrefix + " userName: " + userName + " user_email: " + user_email + " user_phone: " + JSON.stringify(user_phone));
	var headers = {
		'Authorization': 'Token token=' + api_key,
		'user-agent': 'node.js'
	};
		...
		if(user) {
			...
			async.auto({
				...
				if (err) {
					...
					if (res) {
						logger.info(logPrefix + "Returning bad request (400): " + err);
			...
			request.post({
				...
				async.auto({
					...
					if (err) {
						// rollback user that was just created
						var userUrl = apiUrl + "/users/" + user.id;
						request.del({
							uri: userUrl,
							headers: headers
						}, function(err2, reqRes2, body2) {
							logger.error(logPrefix + "Error while adding contacts and notification rules: " + err);
							if (res) {
								logger.info(logPrefix + "Returning bad request (400): " + err);
								res.status(400).json({ "description": err });						
...
function getOrCreateEscalationPolicy(res, apiUrl, api_key, service_name, user_email, user_phone, callback) {
	...
	logger.debug(logPrefix + " service_name: " + service_name + " user_email: " + user_email + " user_phone: " + JSON.stringify(user_phone));
	var headers = {
		'Authorization': 'Token token=' + api_key,
		'user-agent': 'node.js'
	};
...
function getOrCreatePagerDutyService(res, apiUrl, api_key, site_name, service_name, user_email, user_phone, patching, callback) {
	var logPrefix = "[" + logBasePath + ".getOrCreatePagerDutyService] ";
	logger.debug(logPrefix + "site_name:" + site_name + " service_name: " + service_name + " user_email: " + user_email + " user_phone: " + JSON.stringify(user_phone));
	var headers = {
		'Authorization': 'Token token=' + api_key,
		'user-agent': 'node.js'
	};
	...
	request.get({
		...
		if (err && (err.code == "ENOTFOUND" || err.cert)) { // special error handling for invalid site name case
			var message = "Invalid site_name: " + site_name;
			logger.info(logPrefix + "Returning bad request (400): " + message);
		...
		if (handleError(res, err, reqRes, 200, callback, logPrefix, "Verifying if the PagerDuty service already exists for service_name: " + service_name, function(statusCode, errorMessage) {
			if (statusCode == 401/*invalid api_key*/) {
				statusCode = 400/*make it a bad request*/;
				errorMessage = "Invalid api_key: " + api_key; // no message from PagerDuty, create one
				logger.info(logPrefix + "Returning bad request (400): " + errorMessage);
			}
			if (statusCode == 404/*unknown site_name*/) {
				statusCode = 400/*make it a bad request*/;
				errorMessage = "Unknown site_name: " + site_name; // cannot use message from PagerDuty ('Account Not Found') which is confusing
				logger.info(logPrefix + "Returning bad request (400): " + errorMessage);
			}
		...
		if(service) {
					...
					getUserInfo(res, apiUrl, api_key, userId, function(err, email, phone) {
						...
						if (user_email != email || user_phone != phone) {
							return updateEscalationPolicy(res, apiUrl, api_key, service.escalation_policy.id, escalationPolicy.escalation_rules, user_email, user_phone, patching, function(err, escalation_policy) {
								if (err) 
									return callback(err, null);
								return callback(null, service, true);
							});
						}
			...
			if (!user_email) {
				// default to unknown since not mandatory
				user_email = "unknown_user@email.com";
			}
			getOrCreateEscalationPolicy(res, apiUrl, api_key, service_name, user_email, user_phone, function(err, escalation_policy) {
				...
				request.post({
					...
					if (handleError(res, err, reqRes, 201, callback, logPrefix, "Creating the PagerDuty service for service_name: " + service_name))
						return;
					return callback(null, body.service, false);
				});
			});
		}
	});
...
function handleError(res, err, reqRes, expectedStatusCode, callback, logPrefix, logMessage, unexpectedStatusCodeHandler) {
	if (err) {
		logger.error(logPrefix + logMessage + " failed with the following error from PagerDuty: " + JSON.stringify(err, null, 2));
		if (res) {
		...
		logger.debug(logPrefix + logMessage + " failed [" + reqRes.statusCode + "] with the following error from PagerDuty: " + JSON.stringify(reqRes, null, 2));
		...
		if (res) {
			if (unexpectedStatusCodeHandler) {
				...
				logger.info(logPrefix + "Returning " + reqRes.statusCode + ": " + errorMessage);
...
function postAlert(service_key, description, callback) {
	var logPrefix = "[" + logBasePath + ".postAlert] ";
	logger.debug(logPrefix + "service_key:" + service_key + " description: " + description);
	...
	request.post({
		...
		// NOTE: PagerDuty returns a 200 even if the service has been deleted, so there is no way to surface this issue at this level
...
function updateEscalationPolicy(res, apiUrl, api_key, escalation_id, escalation_rules, user_email, user_phone, updateUser, callback) {
	...
	logger.debug(logPrefix + "escalation policy: " + escalation_id + " user_email: " + user_email);
	var headers = {
		'Authorization': 'Token token=' + api_key,
		'user-agent': 'node.js'
	};
	...
	getOrCreatePagerDutyUser(res, apiUrl, api_key, userName, user_email, user_phone, function(err, user) {
		...
		getUser(res, apiUrl, api_key, previousPrimaryContactId,  function(err, primaryUser) {
			...
			if (updateUser) {
				if (escalation_rules[0].targets[0].id == user.id)
					return callback(null, null);
				...
				var exists = _.find(escalation_rules, function(escalationRule) {
					var targetExists = _.find(escalationRule.targets, function(target) {
